...
class Songs {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException {  //抛出过程中可能出现的异常

        Songs songs = new Songs();
        List<Song> songsList = songs.getSongs();
        try{
            ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("songs.ser"));
            //之前试过用.txt文件存储序列化过后的歌曲列表，也可以，但是后面看.ser是专门用来存这个的就改了(qwq)
            oos.writeObject(songsList);
        } catch (IOException e){
            e.printStackTrace();
        }

        List<Song> deSerializedSongs;
        ObjectInputStream ois = new ObjectInputStream(new FileInputStream("songs.ser"));
        deSerializedSongs = (List<Song>) ois.readObject();   //进行强制类型转换，为List

        //deserializedSongs.forEach(System.out::println);   //可选的再次打印全列表

        //还是一样执行上个题的筛选和输出
        deSerializedSongs.stream()
